...
export const getUserStats = async (req, res) => {
  try {
    ...
    const sessions = await Session.find({ userId, isActive: false }).sort({
      createdAt: 1,
    });
    ...
    sessions.forEach((sess) => {
      if (sess.summary?.mistakes) {
        sess.summary.mistakes.forEach((mistake) =>
    ...
    const accuracyTrend = quizzes.map((q) => {
      ...
      return {
        date: new Date(q.createdAt).toLocaleDateString(undefined, {
          month: "short",
          day: "numeric",
          hour: "2-digit",
          minute: "2-digit",
